...
def lambda_handler(event, context):
    ...
    try:
        ...
        # LOCK ORDER BEFORE READING CART (fix for race condition)
        try:
            table.update_item(
                Key={"orderId": orderId, "userId": userId},
                UpdateExpression='SET orderStatus = :lock',
            ...
            if e.response['Error']['Code'] == 'ConditionalCheckFailedException':
                return {"status": "err", "msg": "order already being processed"}
            raise
        ...
        if status < 120:
            ...
            if cartTotal <= 0:
                table.update_item(
                    Key={"orderId": orderId, "userId": userId},
                    UpdateExpression='SET orderStatus = :open',
                    ExpressionAttributeValues={':open': 100}
                )
                return {"status": "err", "msg": "Invalid order total: amount must be greater than zero"}
